...
String createPage() {
    // Match the request
  int val1 = digitalRead(GPIO_D1_PIN);
  int val2 = digitalRead(GPIO_D2_PIN);
...
void handleLightOff2() {
  digitalWrite(GPIO_D2_PIN, HIGH);
  
  server.send(200, "text/html", createPage());
}

void handleLightOn2() {
  digitalWrite(GPIO_D2_PIN, LOW);
...
//    MDNS.addService("https", "tcp", 443);
  ...
  server.on("/1/on", HTTP_GET, handleLightOn1);
  server.on("/1/off", HTTP_GET, handleLightOff1);
  server.on("/2/on", HTTP_GET, handleLightOn2);
  server.on("/2/off", HTTP_GET, handleLightOff2);
